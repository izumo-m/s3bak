...
| a directory where the record is a file | as without `-u`: the stale file record stays until `push --delete` retires it — beside a directory record if the contents are pushed, which makes the entry [unrestorable](08-recovery-troubleshooting.md#unrestorable-a-file-and-a-directory-at-one-path) until then | kept when its modification time is newer; otherwise a conflict, since a directory is never replaced; `--delete` never offers what is inside it |
